...
# Look for google service account (.json files) in the current directory
json_files = [f for f in os.listdir(".") if f.endswith(".json")]

if len(json_files) != 1: # Expected there is only one .json file 
...
# text recognition with Google Vision API
...
def padd_init(lang='en'):
    ocr = PaddleOCR(
        lang=lang, # Specify lang recognition model with the lang parameter
...
# text recognition with paddle OCR
